...
class SnapToBlock(// Maxim blocks to move during most vigorous fling.
    ...
    private var mSnapBlockCallback: SnapBlockCallback? =
        null
    ...
    // LTR/RTL helper
    private var mLayoutDirectionHelper: LayoutDirectionHelper? =
        null
    ...
    override fun findTargetSnapPosition(
        ...
        if (velocityX != 0) {
            return mLayoutDirectionHelper?.getPositionsToMove(lm, mScroller!!.finalX, mItemDimension)?:0
        }
        return if (velocityY != 0) {
            mLayoutDirectionHelper?.getPositionsToMove(lm, mScroller!!.finalY, mItemDimension)?:0
